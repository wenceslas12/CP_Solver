<script>
    import {nodePert, pathsPERT} from "../../routes/store/PertStore.js";
    ...
    let numOfTicks = $nodePert.find(x => x.pathsFrom.length === 0).actualRightValue + 1; // Počet bodů na ose, můžete změnit podle potřeby
    let axisStart;
    let rectElements = [];

    //Find the longest name of activity and set axisStart
    const longestPathElement = $pathsPERT.reduce((prev, current) => {
    ...
    const sortedPaths = $pathsPERT.sort((a, b) => {
    ...
    const sortedPathsWithout0DurationsPath = sortedPaths.filter(x => x.meanValue !==0);
    ...
    function showDialogMouseOver(i, path) {
        try {
            ...
            dialogDurationOfActivity = path.meanValue;
...
<dialog id="confirmation-dialog">
    ...
    <p>Mean Value: <b>{dialogDurationOfActivity}</b></p>
