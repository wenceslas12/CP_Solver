<script>
    ...
    import {activityInTable} from "../../routes/store/PertStore.js";
...
<div class="table">
    <Table hoverable={true}>
        <TableHead>
            ...
            <TableHeadCell>Optimistic Estimate</TableHeadCell>
            <TableHeadCell>Most Likely Estimate</TableHeadCell>
            <TableHeadCell>Pessimistic Estimate</TableHeadCell>
            <TableHeadCell>Mean Value</TableHeadCell>
            <TableHeadCell>Dispersion</TableHeadCell>
                ...
                <TableBodyRow>
                    ...
                    <TableBodyCell>
                        <P size="2xl" align="center">
                            {path.optimisticEstimate}
                        </P>
                    </TableBodyCell>
                    <TableBodyCell>
                        <P size="2xl" align="center">
                            {path.mostLikelyEstimate}
                        </P>
                    </TableBodyCell>
                    <TableBodyCell>
                        <P size="2xl" align="center">
                            {path.pessimisticEstimate}
                        </P>
                    </TableBodyCell>
                    <TableBodyCell>
                        <P size="2xl" align="center">
                    ...
                    <TableBodyCell>
                        {#each path.previousActivities as previousPath, i}
                            {#if path.previousActivities.length - 1 === i && $activityInTable.find(x => x.index === previousPath).optimisticEstimate !==0 }
                                {$activityInTable.find(x => x.index === previousPath).nameOfActivity}
                            {:else if $activityInTable.find(x => x.index === previousPath).optimisticEstimate !==0 }
                                {$activityInTable.find(x => x.index === previousPath).nameOfActivity + ", "}
                            {/if}
                        {/each}
                    </TableBodyCell>
                    ...
                    <TableBodyCell>
                        {path.timeReserve === undefined ? "" : path.timeReserve}
                    </TableBodyCell>
                </TableBodyRow>
            {/each}
        </TableBody>
