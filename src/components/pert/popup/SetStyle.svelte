<script>
    ...
    import {nodePert, pathsPERT, style} from "../../../routes/store/PertStore.js";
    ...
    let onMeanValueActivity = $style. onMeanValueActivity;

    function setStyle(){
        //Set r for all circle
        for (let i = 0; i < $nodePert.length; i++) {
            $nodePert[i].r =sizeCircle;
        }
        ...
        style.set({
            ...
            onMeanValueActivity:  onMeanValueActivity
        })
...
<div class="absolute w-1/4 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-4 text-center bg-white bg-opacity-70 rounded-lg">
    ...
    <svg viewBox="0 0 408 202">
        ...
        <line
                ...
                y2={100}
                stroke="black"
                marker-end="url(#arrowSetting)"
                stroke-width={strokeWidth}
        />
        {#if  onMeanValueActivity}
            <!--Path durations-->
            <text
    ...
    <div class="grid">
        ...
        <Toggle checked={ onMeanValueActivity} on:click={() => ( onMeanValueActivity = !onMeanValueActivity)}
        > On/Off mean value
        </Toggle>
    </div>
